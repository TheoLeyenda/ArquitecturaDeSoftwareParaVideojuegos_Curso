...
namespace ZooArchitect.View.Entities
{
    ...
    internal sealed class EntitiesLogicView : IInitable ,ITickable, IDisposable
    {
        ...
        private WorkersLogicView workersLogicView;
        public EntitiesLogicView()
        {
            animalsLogicView = new AnimalsLogicView();
            workersLogicView = new WorkersLogicView();
        ...
        public void Init()
        {
            ...
            workersLogicView.Init();
        }

        public void LateInit()
        {
            animalsLogicView.LateInit();
            workersLogicView.LateInit();
        }

        public void Tick(float deltaTime)
        {
			foreach (EntityView entityView in EntityRegistryView.Entities)
			{
                entityView.Tick(deltaTime);
			}

            animalsLogicView.Tick(deltaTime);
            workersLogicView.Tick(deltaTime);
        }

        public void Dispose()
        {
            animalsLogicView.Dispose();
            workersLogicView.Dispose();
